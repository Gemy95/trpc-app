...
import { ONLINE_STATUS as MERCHANT_ONLINE_STATUS, MERCHANT_STATUS } from '../../common/constants/merchant';
import {
...
export class OrderRepository extends BaseRepository<Order> {
  ...
  public async listOrdersDashboard(query: DashboardOrderQueryDto, branchId?: string) {
    const { limit, page, paginate = true, ...rest } = query;

    const generatedMatch = generateFilters(rest);
    const pagination = !isNaN(page) || !isNaN(limit) ? generatePagination(limit, page) : [];
    ...
    if (generatedMatch['transactionRefId'] && mongoose.Types.ObjectId.isValid(generatedMatch['transactionRefId'])) {
      delete Object.assign(generatedMatch, {
        'transaction._id': new Types.ObjectId(generatedMatch['transactionRefId']),
      })['transactionRefId'];
    }

    const settingOrder = await this.settingRepository.getOne({ modelName: 'Order' });
    const timeAfterDeliveredOrder =
      settingOrder && !isNaN(settingOrder['TimeAfterDeliveredOrder']) ? settingOrder['TimeAfterDeliveredOrder'] : 3;
    const dateTimeAfterDeliveredOrder = new Date(Date.now() - timeAfterDeliveredOrder * 60 * 60 * 1000);
                    ...
                    $match: {
                      $expr: {
                        $and: [
                          ...
                          { $eq: ['$clientId', new mongoose.Types.ObjectId(user._id)] },
                        ],
                      },
                    },
                  },
      ...
      },
      {
        $lookup: {
          ...
          pipeline: [
            ...
            {
              $unwind: { path: '$categoriesIds', preserveNullAndEmptyArrays: true },
            },
          ],
          as: 'products',
        },
          ...
          pipeline: [
            {
              $match: {
                $expr: {
                  $and: [{ $in: ['$_id', '$$tags'] }, { $eq: ['$client_visibility', true] }],
                },
              },
            },
          ],
  ...
  public async getLatestOrder(
    client: string,
  ): Promise<{ success: boolean; message?: string; rated?: boolean; data?: any }> {
    try {
  ...
  public async listOrdersDriver(query: DashboardOrderQueryDto, branchId: string, user: any) {
    ...
    const settingOrder = await this.settingRepository.getOne({ modelName: 'Order' });
    const timeAfterDeliveredOrder = !isNaN(settingOrder['TimeAfterDeliveredOrder'])
    ...
    return {
      ...
      ClientNotRespondOrders: orders?.ClientNotRespondOrders[0]?.ClientNotRespondCount || 0,
      ClientNotDeliveredOrders: orders?.ClientNotDeliveredOrders[0]?.ClientNotDeliveredOrderCount || 0,
      Delivered: orders?.DeliveredOrders[0]?.DeliveredCount || 0,
      page,
      pageCount: Math.ceil(orders?.Count[0]?.count / limit) || 1,
      length: orders?.Count[0]?.count || 0,
          ...
          pipeline: [
            {
              $match: {
                $expr: {
                  $and: [{ $eq: ['$client', '$$clientId'] }, { $eq: ['$active', true] }],
                },
              },
            },
          ],
