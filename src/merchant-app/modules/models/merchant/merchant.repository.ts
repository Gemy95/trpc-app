...
import { ORDER_EXPIRED_STATUS, ORDER_PENDING_STATUS, ORDER_STATUS } from '../../common/constants/order.constants';
...
import { FindDashboardMerchantsStatisticsDto } from '../../merchant/dto/dashboard-merchant-statistics.dto';
import { MerchantQueryDto } from '../../merchant/dto/merchant-query.dto';
import { FindMerchantStatisticsDto } from '../../merchant/dto/merchant-statistics.dto';
...
export class MerchantRepository extends BaseRepository<Merchant> {
          ...
          pipeline: [
            {
              $match: {
                $expr: {
                  $and: [{ $in: ['$branchId', '$$branchId'] }, { $eq: ['$status', ORDER_PENDING_STATUS] }],
                },
              },
            },
          ],
  ...
  async getAllMerchantsWithFilter(query: MerchantQueryDto): Promise<FindAllMerchantType> {
    ...
    if (generatedMatch['ownerMobile']) {
      delete Object.assign(generatedMatch, { 'owner.mobile': generatedMatch['ownerMobile'] })['ownerMobile'];
    }

    if (generatedMatch['ownerEmail']) {
      delete Object.assign(generatedMatch, { 'owner.email': generatedMatch['ownerEmail'] })['ownerEmail'];
    }
  ...
  async getMerchantStatisticsById(merchantId, params: FindMerchantStatisticsDto) {
    const { ...rest } = params;
    const generatedMatch = generateFilters(rest);
    ...
    const result = await this.nModel.aggregate([
          ...
          pipeline: [
            {
              $match: {
                $expr: {
                  $and: [{ $eq: ['$merchantId', '$$localFieldMerchantId'] }, { $eq: ['$isDeleted', false] }],
                },
              },
            },
            {
          ...
          pipeline: [
            {
              $match: {
                $expr: {
                  $and: [{ $in: ['$branchId', '$$branchId'] }, { $ne: ['$status', ORDER_EXPIRED_STATUS] }],
                },
              },
            },
            {
      ...
      },
      {
        $project: {
          ...
          totalMerchantEmployeesCount: { $size: { $ifNull: ['$merchantEmployees', 0] } },
          totalOrdersCount: { $size: { $ifNull: ['$orders', 0] } },
          totalOrdersPrice: {
            $sum: { $ifNull: ['$orders.invoice.total', 0] },
          },
          totalClientsOrdersCount: { $size: { $ifNull: ['$clients_orders', 0] } },
          totalRatesCount: { $size: { $ifNull: ['$ratings', 0] } },
          totalComplaintsAndSuggestionsCount: { $size: { $ifNull: ['$compliant', []] } },
      ...
      result[0]['productsIntoOrders'] =
        result?.[0]?.productsIntoOrders.map((product) => {
          ...
          for (let i = 0; i < result?.[0]?.ordersProductsItems?.length; i++) {
            if (result?.[0]?.ordersProductsItems[i].productId.toString() == product._id.toString()) {
          ...
          pipeline: [
            {
              $match: {
                $expr: {
                  $and: [{ $eq: ['$merchant', '$$localFieldMerchantId'] }, { $eq: ['$is_public', true] }],
                },
              },
            },
            {
  ...
  async getDashboardMerchantsStatistics(params: FindDashboardMerchantsStatisticsDto) {
    const { ...rest } = params;
    const generatedMatch = generateFilters(rest);
          ...
          Result2: [
                ...
                pipeline: [
                  ...
                  },
                  {
                    $match: {
                      $expr: {
                        $or: [
                          { $eq: ['$status', ORDER_STATUS.ORDER_REJECTED_BY_EMPLOYEE_STATUS] },
                          { $eq: ['$status', ORDER_STATUS.ORDER_REJECTED_BY_MERCHANT_STATUS] },
                          { $eq: ['$status', ORDER_STATUS.ORDER_REJECTED_BY_OPERATION_STATUS] },
                          { $eq: ['$status', ORDER_STATUS.ORDER_REJECTED_BY_SHOPPEX_STATUS] },
                        ],
                      },
                    },
                  },
                ...
                pipeline: [
                  ...
                  },
                  {
                    $match: {
                      $expr: {
                        $or: [
                          { $eq: ['$status', ORDER_STATUS.ORDER_CANCELED_BY_MERCHANT_STATUS] },
                          { $eq: ['$status', ORDER_STATUS.ORDER_CANCELED_BY_OPERATION_STATUS] },
                          { $eq: ['$status', ORDER_STATUS.ORDER_CANCELED_BY_CLIENT_STATUS] },
                          { $eq: ['$status', ORDER_STATUS.ORDER_CANCELED_BY_EMPLOYEE_STATUS] },
                          { $eq: ['$status', ORDER_STATUS.ORDER_CANCELED_BY_SHOPPEX_STATUS] },
                        ],
                      },
                    },
                  },
                ...
                pipeline: [
                  {
                    $match: {
                      $expr: {
                        $and: [
                          { $in: ['$branchId', '$$localFieldBranchId'] },
                          { $eq: ['$status', ORDER_STATUS.ORDER_ACCEPTED_STATUS] },
                        ],
                      },
                    },
                  },
                ...
                pipeline: [
                  {
                    $match: {
                      $expr: {
                        $and: [
                          { $in: ['$branchId', '$$localFieldBranchId'] },
                          { $eq: ['$status', ORDER_STATUS.ORDER_ON_WAY_TO_CLIENT] },
                        ],
                      },
                    },
                  },
                ...
                pipeline: [
                  {
                    $match: {
                      $expr: {
                        $and: [
                          { $in: ['$branchId', '$$localFieldBranchId'] },
                          { $eq: ['$status', ORDER_STATUS.ORDER_ARRIVED_TO_CLIENT] },
                        ],
                      },
                    },
                  },
                ...
                pipeline: [
                  {
                    $match: {
                      $expr: {
                        $and: [
                          { $in: ['$branchId', '$$localFieldBranchId'] },
                          { $eq: ['$status', ORDER_STATUS.ORDER_CLIENT_NOT_RESPOND] },
                        ],
                      },
                    },
                  },
                ...
                pipeline: [
                  {
                    $match: {
                      $expr: {
                        $and: [
                          { $in: ['$branchId', '$$localFieldBranchId'] },
                          { $eq: ['$status', ORDER_STATUS.ORDER_CLIENT_NOT_DELIVERED] },
                        ],
                      },
                    },
                  },
            ...
            },
            {
              $project: {
                ...
                totalPendingOrdersCount: { $size: { $ifNull: ['$pendingOrders', 0] } },
                totalReadyOrdersCount: { $size: { $ifNull: ['$readyOrders', 0] } },
                totalAcceptedOrdersCount: { $size: { $ifNull: ['$acceptedOrders', 0] } },
                totalDeliveredOrdersCount: { $size: { $ifNull: ['$deliveredOrders', 0] } },
                totalCanceledOrdersCount: { $size: { $ifNull: ['$canceledOrders', 0] } },
                totalRejectedOrdersCount: { $size: { $ifNull: ['$rejectedOrders', 0] } },
                totalExpiredOrdersCount: { $size: { $ifNull: ['$expiredOrders', 0] } },
                totalOnWayToClientOrdersCount: { $size: { $ifNull: ['$onWayToClientOrders', 0] } },
                totalArrivedToClientOrdersCount: { $size: { $ifNull: ['$arrivedToClientOrders', 0] } },
                totalClientNotRespondOrdersCount: { $size: { $ifNull: ['$clientNotRespondOrders', 0] } },
                totalClientNotDeliveredOrdersCount: { $size: { $ifNull: ['$clientNotDeliveredOrders', 0] } },
                totalOrdersClientsCount: { $size: { $ifNull: ['$clients_orders', 0] } },
                totalReservationsCount: { $size: { $ifNull: ['$reservations', 0] } },
                totalComplaintsAndSuggestionsCount: { $size: { $ifNull: ['$compliant', []] } },
              },
            },
            {
              $group: {
                _id: null,
                totalMerchantEmployeesCount: { $sum: { $ifNull: ['$totalMerchantEmployeesCount', 0] } },
                totalOrdersCount: { $sum: { $ifNull: ['$totalOrdersCount', 0] } },
                totalOrdersPrice: {
                  $sum: { $ifNull: ['$totalOrdersPrice', 0] },
                },
                totalPendingOrdersCount: { $sum: { $ifNull: ['$totalPendingOrdersCount', 0] } },
                totalReadyOrdersCount: { $sum: { $ifNull: ['$totalReadyOrdersCount', 0] } },
                totalAcceptedOrdersCount: { $sum: { $ifNull: ['$totalAcceptedOrdersCount', 0] } },
                totalDeliveredOrdersCount: { $sum: { $ifNull: ['$totalDeliveredOrdersCount', 0] } },
                totalCanceledOrdersCount: { $sum: { $ifNull: ['$totalCanceledOrdersCount', 0] } },
                totalRejectedOrdersCount: { $sum: { $ifNull: ['$totalRejectedOrdersCount', 0] } },
                totalExpiredOrdersCount: { $sum: { $ifNull: ['$totalExpiredOrdersCount', 0] } },
                totalOnWayToClientOrdersCount: { $sum: { $ifNull: ['$totalOnWayToClientOrdersCount', 0] } },
                totalArrivedToClientOrdersCount: { $sum: { $ifNull: ['$totalArrivedToClientOrdersCount', 0] } },
                totalClientNotRespondOrdersCount: { $sum: { $ifNull: ['$totalClientNotRespondOrdersCount', 0] } },
                totalClientNotDeliveredOrdersCount: { $sum: { $ifNull: ['$totalClientNotDeliveredOrdersCount', 0] } },
                totalOrdersClientsCount: { $sum: { $ifNull: ['$totalOrdersClientsCount', 0] } },
                totalReservationsCount: { $sum: { $ifNull: ['$totalReservationsCount', 0] } },
                totalComplaintsAndSuggestionsCount: { $sum: { $ifNull: ['$totalComplaintsAndSuggestionsCount', 0] } },
              },
            },
          ],
          Result3: [
                    ...
                    $match: {
                      $expr: {
                        $and: [
                          ...
                          {
                            $gte: ['$createdAt', { $dateSubtract: { startDate: '$$NOW', unit: 'month', amount: 6 } }],
                          },
                        ],
                      },
                    },
    ...
    const ordersClients = Array.from(
      result?.[0]?.Result3?.[0]?.allOrdersClients?.reduce(
        (m, { createdAt, count }) => m.set(createdAt, (m.get(createdAt) || 0) + count),
        new Map(),
      ),
    ...
    const ordersRevenues = Array.from(
      result?.[0]?.Result4?.[0]?.allRevenues?.reduce(
        (m, { createdAt, revenue }) => m.set(createdAt, (m.get(createdAt) || 0) + revenue),
        new Map(),
      ),
      ([createdAt, revenue]) => ({ createdAt, revenue }),
    )?.sort((a, b) => Date.parse(a?.createdAt) - Date.parse(b?.createdAt));

    const totalRevenues = result?.[0]?.Result4?.[0]?.allRevenues?.reduce((acc, obj) => acc + (obj?.revenue || 0), 0);

    return { ...result[0].Result1[0], ...result[0].Result2[0], ordersClients, ordersRevenues, totalRevenues };
  }
