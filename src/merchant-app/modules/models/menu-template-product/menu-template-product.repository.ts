...
import { CreateMenuTemplateProductDto } from '../../menu-template-product/dto/create-menu-template-product.dto';
...
export class MenuTemplateProductRepository extends BaseRepository<MenuTemplateProduct> {
  constructor(@InjectModel(MenuTemplateProduct.name) private readonly nModel: Model<MenuTemplateProduct>) {
    super(nModel);
  }

  async createOne(createMenuTemplateProductDto: CreateMenuTemplateProductDto) {
    const menuTemplateProduct = await this.prepareMenuTemplateProduct(createMenuTemplateProductDto);

    const menuTemplateProductResponse = await this.create({ ...menuTemplateProduct });
  ...
  async findOne(menuTemplateProductId: string) {
    ...
    if (!menuTemplateProduct) {
      throw new NotFoundException(ERROR_CODES.err_menu_template_product_not_found);
    }
  ...
  async updateOne(menuTemplateProductId: string, updateMenuTemplateProductDto: UpdateMenuTemplateProductDto) {
    const menuTemplateProduct = await this.prepareMenuTemplateProduct(updateMenuTemplateProductDto);

    const isMenuTemplateProductExists = await this.getById(new mongoose.Types.ObjectId(menuTemplateProductId), {});
    if (!isMenuTemplateProductExists) {
      throw new NotFoundException(ERROR_CODES.err_menu_template_product_not_found);
    }
    return this.updateById(menuTemplateProductId, { ...menuTemplateProduct }, { lean: true, new: true }, {});
  }

  async remove(menuTemplateProductId: string) {
    const menuTemplateProduct = await this.getById(new mongoose.Types.ObjectId(menuTemplateProductId), {});
    if (!menuTemplateProduct) {
      throw new NotFoundException(ERROR_CODES.err_menu_template_product_not_found);
    }
  ...
  async prepareMenuTemplateProduct(
    createMenuTemplateProductDto: CreateMenuTemplateProductDto | UpdateMenuTemplateProductDto,
  ) {
    const menuTemplateProduct = new MenuTemplateProduct();

    menuTemplateProduct.name = createMenuTemplateProductDto.nameArabic;
    menuTemplateProduct.description = createMenuTemplateProductDto.descriptionArabic;
      ...
      translation: [
        {
          ...
          description: createMenuTemplateProductDto?.mainImage?.descriptionEnglish,
        },
      ],
    };
    menuTemplateProduct.price = createMenuTemplateProductDto.price;
    menuTemplateProduct.preparationTime = createMenuTemplateProductDto.preparationTime;
    ...
    menuTemplateProduct.mealsTime = createMenuTemplateProductDto?.mealsTime?.map((ele) => {
      return { name: ele, times: getMealsTime(ele) };
    });
    menuTemplateProduct.quantity = createMenuTemplateProductDto?.quantity;
    menuTemplateProduct.tagsIds = createMenuTemplateProductDto?.tagsIds?.map((ele) => {
      return new mongoose.Types.ObjectId(ele);
    });
    menuTemplateProduct.isDeleted = false;
    menuTemplateProduct.menuTemplateProductGroupsIds = createMenuTemplateProductDto?.menuTemplateProductGroupsIds?.map(
      (ele) => {
        return new mongoose.Types.ObjectId(ele);
      },
    );
