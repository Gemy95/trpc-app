...
import { Owner } from '../../../libs/database/src/lib/models/owner/owner.schema';
...
import { OwnerFiltersQuery } from '../../modules/dashboard/dto/owner-filters.dto';
...
export class OwnerRepository extends BaseRepository<Owner> {
  constructor(
    @InjectModel('Owner')
    private readonly nModel: Model<Owner>,
  ) {
    super(nModel);
  }

  async create(data): Promise<Owner> {
  ...
  async updateOne(query, data): Promise<Owner> {
    return this.nModel
    ...
    return this.nModel.aggregate([
      {
        $match: {
          ...
          type: ClientOwnerTypesDetailsEnum.OWNER,
        },
      },
      {
        $lookup: {
  ...
  async getForDashboard(filters: OwnerFiltersQuery) {
