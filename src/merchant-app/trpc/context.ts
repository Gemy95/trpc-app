...
export const createContext = async ({ req, res }: CreateExpressContextOptions): Promise<Context> => {
  ...
  try {
    if (req?.headers?.authorization) {
      ...
      if (!header) {
        throw new TRPCError({ message: ERROR_CODES.err_auth_token_is_missing, code: 'UNAUTHORIZED' });
      }

      const parts = header.split(' ');
      if (parts.length !== 2 || parts[0] !== 'Bearer') {
        throw new TRPCError({ message: ERROR_CODES.err_auth_token_is_invalid, code: 'UNAUTHORIZED' });
      }
      ...
      if (jwtPayload && jwtPayload?.['exp'] < Date.now() / 1000) {
        throw new TRPCError({ message: ERROR_CODES.err_auth_token_is_expired, code: 'UNAUTHORIZED' }); // need update to use custom error
      }
